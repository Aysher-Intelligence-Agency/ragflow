    ...
    def execute(self) -> tuple[int, list]:
        ...
        for i in range(0, len(records), batch_size):
            ...
            for tenant_id, llm_factory in batch:
                ...
                values.append(f"('{record_id}', '{llm_factory}', '{tenant_id}', {current_ts}, FROM_UNIXTIME({current_ts}), {current_ts}, FROM_UNIXTIME({current_ts}))")
    ...
    def execute(self) -> tuple[int, list]:
        ...
        for i in range(0, len(records), batch_size):
            ...
            for tenant_id, llm_factory, api_key, status, provider_id in batch:
                ...
                values.append(
                    f"('{record_id}', '{instance_name}', '{provider_id}', '{api_key_escaped}', '{status_val}', {current_ts}, FROM_UNIXTIME({current_ts}), {current_ts}, FROM_UNIXTIME({current_ts}))"
                )
...
def run_migration(config: MigrationConfig, stages: list, dry_run: bool = True, create_table_only: bool = False):
...
""",
    ...
    run_migration(config=config, stages=stages, dry_run=dry_run, create_table_only=create_table_only)
